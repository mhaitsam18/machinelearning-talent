...
try:
    ...
    for datamodel in ModelHasil:
        ...
        # print(mode)
    sql_select_query_model2 = '''SELECT name from models WHERE type = 'randomforest' ORDER BY id desc limit 1'''
    ...
    loaded_model = joblib.load(mode)  # neighbors
    loaded_model2 = joblib.load(mode2)  # randomforest

    features = readfile.loc[:, 'MD': 'SISTEMATIKA_KERJA'].values
    scaler = preprocessing.StandardScaler().fit(features)
    scaled_feature = scaler.transform(features)
    result = loaded_model.predict(scaled_feature)  # neighbors
    result2 = loaded_model2.predict(scaled_feature)  # randomforest
    hasil = pd.DataFrame(result)  # neighbors
    hasil2 = pd.DataFrame(result2)  # random forest
    nip = readfile['NIP']
    name = readfile['NAMA']
    ...
    # Hasil to excel
    savenama = 'neighbors_randomforest_hasil_' + datacoba
